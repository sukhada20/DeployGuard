...
- [x] **ANOM-01**: System detects post-deployment anomalies by comparing error rate, latency, crash rate, CPU, memory, restart count, and request rate against pre-deployment baselines.
- [x] **ANOM-02**: Deploy Monitor Agent continuously polls Cloud Monitoring and Cloud Logging for signals after each deployment event.
- [x] **ANOM-03**: Anomaly detection produces a structured signal (severity, evidence list, affected metrics, confidence) that feeds the decisioning pipeline.
...
- [x] **FLEET-03**: Agents enforce separation of duties — the Decision Agent cannot directly execute rollback; it requests approval from the Rollback Agent via gateway.
...
- [x] **GOV-02**: Automatic rollback requires all policy checks to pass: confidence threshold ≥ 0.80, severity HIGH or CRITICAL, deployment age < 30 min, stable prior version available, environment policy allows auto-rollback.
- [x] **GOV-03**: Every decision produces an auditable trace including evidence, confidence score, policy check results, authorization outcome, selected action, and execution result.
- [x] **GOV-04**: An Agent Gateway enforces identity verification, permission checks, environment constraints, and policy evaluation for all sensitive tool calls.
...
- [x] **MEM-03**: Stored incident data minimizes sensitive fields; log snippets are sanitized before persistence.

### Security & Safety

- [x] **SEC-01**: Production logs and external data are screened or sanitized before reaching LLM reasoning or tool-use paths (prompt injection prevention).
- [x] **SEC-02**: Model Armor protections are applied to Gemini API calls for sensitive-data leakage prevention.
- [x] **SEC-03**: Sensitive tool calls (rollback execution, Firestore writes) require gateway authorization and are logged in the decision trace.

### Rollback & Recovery

- [x] **ROLL-01**: Rollback Agent executes approved rollback via Cloud Deploy, targeting the last known stable version.
- [x] **ROLL-02**: After rollback, Deploy Monitor Agent re-checks metrics to verify recovery; outcome is written to workflow state and triggers Postmortem Agent.
- [x] **ROLL-03**: Rollback is blocked when policy checks fail (low confidence, insufficient deployment age, no stable version, environment restriction).

### Postmortem & Observability

- [x] **POST-01**: Postmortem Agent generates an auditable postmortem document after each recovery or failed rollback attempt, capturing timeline, evidence, decisions, actions, and outcomes.
- [x] **POST-02**: Operator dashboard provides fleet overview, deployment health status, agent activity log, decision traces, and agent registry view.
- [x] **POST-03**: All agent actions emit OpenTelemetry traces that can be correlated with Cloud Monitoring metrics and Cloud Logging entries.

### Demo & Verification

- [x] **DEMO-01**: The system supports a convincing end-to-end demo flow: healthy deployment → injected metric failure → anomaly detection → investigation → governed rollback decision → recovery verification → postmortem.
