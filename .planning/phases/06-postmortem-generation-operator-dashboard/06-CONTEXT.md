...
3. **Operator Dashboard Frontend**: React + Vite + TypeScript SPA in `web/` styled with Tailwind CSS, featuring a dark-mode SRE command center theme with 4 main views:
...
- **D-05:** **React + Vite + TypeScript in `web/`** — Modern SPA located in `web/` built with React 18/19, TypeScript, Tailwind CSS, Lucide icons, and TanStack Query, compiling to `web/dist`. — **Reversibility:** costly — standardizes the frontend architecture and build pipeline.
