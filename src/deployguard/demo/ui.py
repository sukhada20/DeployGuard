...
def print_header(title: str, subtitle: str = "") -> None:
    ...
    print(f"{FG_BRIGHT_CYAN}║{BOLD}{FG_BRIGHT_WHITE} {title.center(width - 2)} {RESET}{FG_BRIGHT_CYAN}║{RESET}")
    if subtitle:
        print(f"{FG_BRIGHT_CYAN}║{DIM}{FG_WHITE} {subtitle.center(width - 2)} {RESET}{FG_BRIGHT_CYAN}║{RESET}")
    print(f"{FG_BRIGHT_CYAN}╚{'═' * width}╝{RESET}")
    print()


def print_stage_banner(stage_num: int, title: str, agent_name: str, color: str = FG_BRIGHT_CYAN) -> None:
    width = 76
    print(f"\n{color}{'━' * width}{RESET}")
    print(f"{color}{BOLD} STAGE {stage_num:02d} ► {title.upper()}{RESET}  {DIM}[Agent: {agent_name}]{RESET}")
    print(f"{color}{'━' * width}{RESET}\n")


def print_agent_thought(agent_name: str, thought: str, color: str = FG_BRIGHT_MAGENTA) -> None:
    print(f"  {color}┌─ {BOLD}{agent_name}{RESET}{color} (Reasoning & Policy Check) {'─' * max(1, 52 - len(agent_name))}┐{RESET}")
    for line in thought.strip().split("\n"):
        print(f"  {color}│{RESET} {line}")
    print(f"  {color}└{'─' * 72}┘{RESET}\n")


def print_metric_table(baseline: dict[str, float], anomalous: dict[str, float], recovered: dict[str, float] | None = None) -> None:
    ...
    for metric, base_val in baseline.items():
        ...
        if recovered is not None:
            ...
            rec_status = f"{FG_BRIGHT_GREEN}NORMALIZED{RESET}" if abs(rec_val - base_val) <= (base_val * 0.15) else f"{FG_BRIGHT_RED}ANOMALOUS{RESET}"
...
def print_security_alert(scenario: str, details: str, passed: bool = True) -> None:
    badge = f"{FG_BRIGHT_GREEN}🛡️  SECURITY GATE ENFORCED{RESET}" if passed else f"{FG_BRIGHT_RED}🚨 SECURITY BREACH DETECTED{RESET}"
...
def prompt_step(stage_name: str, interactive: bool = True) -> None:
    if interactive:
        try:
            print(f"{FG_BRIGHT_YELLOW}► Press [Enter] to continue to {stage_name}...{RESET}", end="", flush=True)
            input()
            print()
