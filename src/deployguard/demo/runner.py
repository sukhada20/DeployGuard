...
class DemoRunner:
    ...
    async def run_full_lifecycle(self) -> DeploymentWorkflowState:
        ...
        print(f"  [Release Engine] Injected Version: {self.target_version} (Candidate Rollout)")
        print(f"  [Release Engine] Environment: {self.environment}")
        print(f"  [Release Engine] Baseline Status: Stable (Error Rate 1.0%, Latency p95 95ms)\n")

        await broadcaster.broadcast("deployment_initiated", {
            "deployment_id": self.session_id,
            "service_name": self.service_name,
            "version": self.target_version,
            "environment": self.environment,
        ...
        # ---------------------------------------------------------------------
        print_stage_banner(2, "Metric Failure Injection", "Telemetry Harness", FG_BRIGHT_RED)
        print("  [Simulator] Injecting production regression (memory leak + thread exhaustion)...")
        ...
        await broadcaster.broadcast("metric_tick", {
            "service_name": self.service_name,
        ...
        # ---------------------------------------------------------------------
        print_stage_banner(3, "Anomaly Detection & Baseline Diff", "DeployMonitorAgent", FG_BRIGHT_CYAN)
        ...
        await broadcaster.broadcast("anomaly_detected", {
            "deployment_id": self.session_id,
            "severity": "CRITICAL",
            "confidence": 0.96,
            "anomalies": state.anomaly_signal.affected_metrics if state.anomaly_signal else [],
        })
        ...
        # ---------------------------------------------------------------------
        print_stage_banner(4, "Incident Memory Vector Search", "IncidentMemoryAgent", FG_BRIGHT_MAGENTA)
        ...
        await broadcaster.broadcast("memory_retrieved", {
            "deployment_id": self.session_id,
            "matched_incidents": 1,
        ...
        # ---------------------------------------------------------------------
        print_stage_banner(5, "Governed Decision & Policy Gate", "DecisionAgent", FG_BRIGHT_YELLOW)
        ...
        await broadcaster.broadcast("decision_evaluated", {
            "deployment_id": self.session_id,
            "action": "rollback",
            "confidence": 0.94,
            "policy_passed": True,
        ...
        # ---------------------------------------------------------------------
        print_stage_banner(6, "Governed Rollback Execution", "RollbackAgent", FG_BRIGHT_RED)
        ...
        await broadcaster.broadcast("rollback_initiated", {
            "deployment_id": self.session_id,
        ...
        # ---------------------------------------------------------------------
        print_stage_banner(7, "Recovery Verification Loop", "DeployMonitorAgent", FG_BRIGHT_GREEN)
        print("  [Simulator] Rollback completed. Simulating metric normalization across pods...")
        ...
        print_metric_table(self.baseline_metrics, self.anomalous_metrics, self.recovered_metrics)
        ...
        await broadcaster.broadcast("recovery_verified", {
            "deployment_id": self.session_id,
            "verdict": "recovered",
            "samples_checked": 3,
        })
        ...
        # ---------------------------------------------------------------------
        print_stage_banner(8, "Postmortem & Root Cause Synthesis", "PostmortemAgent", FG_BRIGHT_CYAN)
        ...
        if state.postmortem_report:
            ...
            for line in state.postmortem_report.to_markdown().split("\n")[:22]:
                print(f"  {line}")
            print(f"  {DIM}... [Full report stored in Firestore collection 'postmortems']{RESET}")
            print(f"  {DIM}{'─' * 72}{RESET}\n")

        await broadcaster.broadcast("postmortem_generated", {
            "deployment_id": self.session_id,
            "report_id": state.postmortem_report.report_id if state.postmortem_report else "",
            "outcome": "recovered",
        })

        print(f"{FG_BRIGHT_GREEN}✨ DEMONSTRATION COMPLETE: Full autonomous lifecycle verified successfully!{RESET}\n")
        return state
