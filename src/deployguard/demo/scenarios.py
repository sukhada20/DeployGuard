...
async def run_gateway_denial_scenario(interactive: bool = True) -> bool:
    ...
    print(
        "  [Simulator] Decision Agent attempting direct execution of deployment.rollback..."
    )
    ...
    assert not is_authorized, (
        "Decision agent must not be allowed to execute rollback directly!"
    )
...
Audit Trace: Logged to Firestore & OpenTelemetry audit span"""
    ...
    print(
        "  [Result] Direct rollback attempt was intercepted and neutralized by Agent Gateway."
    )
...
async def run_prompt_injection_scenario(interactive: bool = True) -> bool:
    ...
    print(
        "  [Simulator] Ingesting poisoned log payload from untrusted production stream:"
    )
    ...
    print_stage_banner(
        2, "Sanitization & PII Redaction", "Model Armor / Sanitizer", FG_BRIGHT_GREEN
    )
    ...
    details = """Original Payload: 215 bytes containing system override & API key
...
Model Armor Status: Passed clean tokens to Gemini reasoning context"""
    ...
    print(
        "  [Result] Prompt poisoning attempt neutralized before reaching LLM reasoning pipeline."
    )
    print(
        "           Decision Agent evaluates genuine telemetry indicators without coercion.\n"
    )
    return True
